<template>
	<view class="l-notice-bar" :class="['l-notice-bar--' + type]" :style="[styles, lStyle]">
		<view class="l-notice-bar__left" v-if="$slots['leftIcon'] || prefixIcon">
			<slot name="leftIcon">
				<image src="/static/home/horn.svg" mode="" class="level_img"></image>
			</slot>
		</view>
		...
		<!-- 垂直滚动区域：新增 disable-touch 属性 -->
		<swiper data-swiper class="l-notice-bar__wrap" v-if="vertical && marquee" 
			:vertical="true" :autoplay="true" :interval="interval" 
			:disable-touch="true"> <!-- 禁用用户手动滑动 -->
