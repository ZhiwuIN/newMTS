		...
		<view class="ei-calendar__container">
			<view class="ei-calendar__container__header">
				<view class="ei-calendar__container__header__date">
					...
					<!-- <view class="ei-calendar__container__header__date__btn" hover-class="is-hover"
						:hover-start-time="20" :hover-stay-time="70" @click="toYear(-1)">
					...
					<!-- <view class="ei-calendar__container__header__date__edge today" hover-class="is-hover"
						:hover-start-time="20" :hover-stay-time="70" @click="toYear(0)">今天</view> -->
				</view>
			...
			<swiper class="ei-calendar__container__content" vertical :current="currentIndex" :duration="duration"
				...
				<swiper-item class="ei-calendar__container__content__item" v-for="(item, itemIndex) in showMonthList"
					:key="itemIndex">
					<view class="ei-calendar__container__content__item__bg">{{ item.month }}</view>
					<view v-for="(row, index) in item.days" :key="index"
						class="ei-calendar__container__content__item__row">
						<view class="ei-calendar__container__content__item__col"
							:class="{ 'is-range': type === 'range' }" v-for="day in row" :key="day.format">
							<ei-calendar-item :day="day" :cell-class="cellClass" :type="type"
								:selected-value="selectedValue" :selection="selection" @click1="onDayClick">
							</ei-calendar-item>
						</view>
					</view>
				</swiper-item>
...
export default {
	...
	computed: {
		currentDateShow() {
			return this.currentDate ? `${this.currentDate.replace('/', '-')}` : '';
		}
	},
	watch: {
		visible(val) {
	...
	methods: {
		...
		getMonthDays(dateStr) { // 获取该年月的日期信息
			...
			for (let i = 1; i <= lastDateOfMonth; i++) {
				...
				//普通日期
				// 当天
				const isToday = i === new Date().getDate() && month === new Date().getMonth() && +year === new Date()
					.getFullYear();
				const dayOption = this.getDayOption({
					year: year,
					month: month,
		...
		getResultValue() {
			if (this.type === 'single') {
				return this.format && this.selectedValue ? EDate.format(this.selectedValue, this.format) : new Date(
					this.selectedValue);
			} else {
				return this.format ? this.selection.map(item => EDate.format(item, this.format)) : this.selection.map(
					item => new Date(item));
			}
		},
		doSubmit() {
