<template>
	<view>
		<view :style="navStyle" class="custom-navbar">
			<!-- <image :lazy-load="true" :src="logoSrc" class="left-icon logo" v-if="isHome">
			</image> -->
			<view class="logo_text" v-if="isHome">{{ webTitle }}</view>
			...
			<image :lazy-load="true" style="width: 48rpx;height: 48rpx;" src="/static/BlackStars/explain.png"
				class="right-icon isFinancePage" v-if="isBlackStars"
				@click="toApplication('/pages/BlackStarsPage/explain')">
			</image>
...
export default {
	...
	setup() {
		...
		onPageScroll((e) => {
			instance?.proxy?.setNavbarBackground(e.scrollTop || 0)
		})
	},
	props: {
		title: {
	...
	methods: {
		...
		setNavBarInfo() {
			...
			uni.getSystemInfo({
				success: (res) => {
					this.mtop = (this.statusBarHeight / 2 + this.navBarHeight) + 24
					this.pageH = res.windowHeight * 2
					this.$emit('mtop', this.mtop)
				}
			});
		},
		setNavbarBackground(scrollTop = 0) {
			this.currentScrollTop = scrollTop
			if (this.whiteBackgroundOnScroll && scrollTop > this.whiteBackgroundScrollTop) {
				this.scrollBackgroundStr = '#FFFFFF'
			} else {
				this.scrollBackgroundStr = this.getDefaultBackground()
			}
		},
		getDefaultBackground() {
			return this.backgroundStr && this.backgroundStr !== "''" ? this.backgroundStr : 'transparent'
		},
	},
	mounted() {
