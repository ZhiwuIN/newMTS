<template>
	<view class="hao-bigbox" :style="toStyle">
		<!-- 搜索 -->
		<view class="hao-search" v-if="isSearch">
			<view class="hao-searchCent">
				...
				<input type="text" v-model="inputKey" @input="searchInput" :placeholder="$t('请输入搜索信息')"
...
<script>
	export default {
		...
		// 2. 修改 mounted 钩子，只初始化基础样式
		mounted() {
			this.afterConverHH = uni.upx2px(this.indexEachHH);
			this.$nextTick(() => {
				this.toStyle = `top: ${this.e}rpx;`;
				// 此时 cityList 可能还未加载，交给 watch 处理
			});
		},
		methods: {
			//回顶部
