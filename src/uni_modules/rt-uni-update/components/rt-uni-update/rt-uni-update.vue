		...
		<view class="content botton-radius">
			<view class="content-top">
				<view class="content-top-text">
					<text class="">{{$t('发现新版本')}}</text>
					<text>v{{data.edition_name}}</text>
					<text class="version">{{$t('当前版本')}}：{{version}}</text>
				</view>
			...
			<view class="content-body">
				...
				<view class="title"><text>{{$t('更新内容')}}</text></view>
				<view class="body">
					<scroll-view class="box-des-scroll" scroll-y="true"><rich-text
							:nodes="data.describe"></rich-text></scroll-view>
				</view>
				<view class="footer flex-center">
					<view class="progress-box flex-column" v-if="!updateBtn">
						<progress class="progress" border-radius="35" :percent="percent" activeColor="#3DA7FF" show-info
							stroke-width="10" />
						<!-- <u-line-progress :striped="true" :percent="percent" :striped-active="true"></u-line-progress> -->
						<view><text class="fs24">{{$t('正在下载')}} ({{downloadedSize}}/{{packageFileSize}}M)</text></view>
					</view>

					<button class="content-button" style="border: none;color: #fff;" plain @click="confirm"
						v-if="updateBtn">{{$t('立即升级')}}</button>
				</view>
			</view>
