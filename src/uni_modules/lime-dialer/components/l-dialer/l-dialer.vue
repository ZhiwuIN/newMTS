<template>
	<view class="l-dialer" :style="getStyle">
		<view class="l-dialer__inner" :style="getDialStyle">
			...
			<view class="l-dialer__inner-wrap" :style="styleOpt.borderColor && (' border: 14rpx solid ' + styleOpt.borderColor)">
				<view class="l-dialer__inner-item" v-for="(item, index) in prizeList" :key="index" :style="[getRotateAngle(index)]">
					<view class="l-dialer__inner-content" :style="[getCorrectAngle(index)]">
						...
						<block v-else>
							<view class="l-dialer__inner-name" :style="[{fontSize: styleOpt.fontSize, color: styleOpt.color}]">{{ item.prizeName }}</view>
							<!-- <image class="l-dialer__inner-img" :src="item.image"></image> -->
						</block>
					</view>
				</view>
			</view>
...
export default {
	...
	computed: {
		...
		getRotateAngle() {
			return index => {
				const style = {
					...
					border: `${this.getStyleOpt.borderColor && '2rpx solid ' + this.getStyleOpt.borderColor }`
				}
		...
		getStyle() {
			...
			return `width: 486rpx; height: 486rpx;`;
		},
		getDialStyle() {
			return `
