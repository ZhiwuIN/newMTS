<template>
	<customnavbar :title="$t('pages.message')">
		...
		<listbottom :hasMore="hasMore" :loading="loading" :noData='nodata' image="/static/default/No news.png"></listbottom>
		...
		methods: {
			getNotice() {
				...
				messageListApi(this.page).then((res) => {
					this.loading = false
					if (this.page.pageNum == 1) this.msgList = res.rows || []
					else this.msgList.concat(res.rows)
