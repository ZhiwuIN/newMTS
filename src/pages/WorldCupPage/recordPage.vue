                ...
                <scroll-view scroll-y :refresher-enabled="true" :refresher-triggered="isRefreshing"
                    ...
                    <view class="list-item" v-for="(item, index) in billsList" :key="index">
                        <view class="item-left">
                            <view class="item-left-t1">{{ currency }} {{ item.rewardAmount }}</view>
                            <view class="item-left-t2">{{ item.title }}</view>
                        </view>
                        <view class="item-right">
                            <view class="item-right-t1">{{ item.claimedAt }}</view>
                            <view class="item-right-t2">{{ item.track }}</view>
                        </view>

                    </view>
                    <listbottom :hasMore="hasMore" :loading="loading" :noData='nodata'
                        image="/static/default/NoContent.png"></listbottom>
                </scroll-view>
                <!-- </view> -->
...
export default {
    ...
    methods: {
        ...
        getList() {
            ...
            weeklyClaimListApi(this.page).then((res) => {
                this.loading = false
