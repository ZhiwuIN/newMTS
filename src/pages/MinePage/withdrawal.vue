		...
		<customnavbar :title="$t('pages.withdrawal')">
			...
			<view class="withdrawal-container">
				<view>
						...
						<view>{{ $t('Withdrawal.Ratio:') }}
							...
							{{ $t('withdrawalRules.feeMessageA') + (withdrawalInfo.fee ||
								'-') + $t('withdrawalRules.feeMessageB') + (withdrawalInfo.dailyTimes ||
									'-') + $t('withdrawalRules.feeMessageC') }}
						</view>
					</view>
					<!-- 提现规则 -->
					<view class="step2-t1">
						{{ $t('Deposit.WithdrawRules') }}:
					</view>
					<view class="step2-text">
						{{ withdrawalInfo.ruleDescription }}
					</view>
					...
					<!-- 提现详细规则 -->
					<view class="step2-t4" v-html="withdrawalInfo.detailedRules">
					</view>
				</view>
	...
	methods: {
		...
		getWithdrawalInfo() {
			...
			withdrawalInfoApi().then((res) => {
				...
				if (this.withdrawalInfo.detailedRules) {
					this.withdrawalInfo.detailedRules = formatRichText(this.withdrawalInfo.detailedRules)
				}
