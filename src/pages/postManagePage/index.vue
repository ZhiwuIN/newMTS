<template>
	<customnavbar :backgroundStr="navbarBackground" @mtop="mtop" class="page" :isPositionManageBlack="true">
		<!-- :whiteBackgroundOnScroll="activeHeadline !== 'grade'" -->
		<!-- :class="{ black: activeHeadline === 'grade' }" -->
		<view class="top-bg" :style="topStyle"></view>
		<view class="content">
			<view class="profile-card">
				<image class="card-crown" src="/static/level_img2.png" mode="aspectFit" />
				<view class="profile-head">
					...
					<view class="profile-info">
						...
						<view class="rank" :class="{ 'node': !userInfo.position }">
							<image src="/static/level_img2.png" mode="aspectFit" />
							{{ userInfo.positionName }}
						</view>
					</view>
				</view>
				<view class="stats">
					<view><strong>{{ subordinateInformation?.lv1Count || 0 }}</strong><text>Mes subordonnes</text>
					</view>
					<view><strong>{{ subordinateInformation?.totalCount || 0 }}</strong><text>Nombre total
							d'equipes</text></view>
					<view><strong>{{ nextJobName }}</strong><text>Poste suivant</text></view>
					<view><strong>{{ currentJob?.assessmentDay }}</strong><text>Evaluation Day</text></view>
				</view>
			</view>

			<view class="section-title">Avantages actuels</view>
			<view class="benefits">
				<view class="benefit wide"><text>Salaire de base</text><strong>{{ currentBenefitsJob.salary }}{{
					currency
				}}</strong></view>
				<view class="benefit"><text>Cycle de paie</text><strong>{{ currentBenefitsJob.payCycle }}</strong>
				</view>
				<view class="benefit"><text>Payday</text><strong>{{ currentBenefitsJob.assessment }}</strong></view>
			</view>

			<!-- 职位考核 -->
			<view class="section-title">This week's assessment</view>
			<view>

			</view>

			<view class="section-title">Details du poste</view>
			<scroll-view class="job-scroll" scroll-x :scroll-left="jobScrollLeft" scroll-with-animation>
				<view class="job" :class="{ active: activeJobIndex === index }" v-for="(item, index) in jobs"
					:key="item.id" @click="selectJob(index)">
					<image :src="item.image" mode="aspectFit" />
					<view><strong>{{ item.name }}</strong></view>
				</view>
			</scroll-view>

			<view class="conditions">
				<view class="triangle" :style="{ left: triangleLeft + 'rpx' }"></view>
				<view class="condition-title">Conditions d'application</view>
				<view class="condition-box">
					<!-- <view class="job-requirement">{{ currentJob.jobRequirements || '--' }}</view> -->
					<view class="progress-row" :class="currentJob?.applicationType"
						v-for="condition in currentJob?.conditions" :key="condition.label">
						<view class="progress-label">
							<text>{{ condition.label }}</text>
							<em>{{ condition.done }}/{{ condition.total }}</em>
						</view>
						<view class="track">
							<view :style="{ width: conditionPercent(condition) + '%' }"></view>
						</view>
					</view>
					<!-- 申请按钮 -->
					<view v-if="canShowApply(currentJob)" class="disabled" @click="positionApply(currentJob)"
						:class="{ reached: canApply(currentJob) }">
						{{ canApply(currentJob) ? 'Apply For' : 'Pas encore atteint' }}
					</view>
				</view>

				<view class="section-title post-title">Avantages du poste</view>
				<view class="benefits post">
					<view class="benefit wide"><text>Salaire de base</text><strong>{{ currentJob?.salary }}{{ currency
					}}</strong>
					</view>
					<view class="benefit"><text>Cycle de paie</text><strong>{{ currentJob?.payCycle }}</strong></view>
					<view class="benefit"><text>Payday</text><strong>{{ currentJob?.assessment }}</strong></view>
				</view>
			</view>
		</view>
	</customnavbar>
...
export default {
	...
	computed: {
		...
		headlineWaveWidth() {
			const text = this.activeHeadline === 'position' ? `Position: ${this.currentJob.name}` : `Grade: ${this.currentJob.level}`
		...
		normalizeJob(item = {}) {
			...
			if (assessmentRequirement) {
				...
				const assessmentLabel = typeof assessmentRequirement === 'object'
					? assessmentRequirement.label || assessmentRequirement.name || assessmentRequirement.requirements || 'Assessment requirements'
	...
	onPageScroll(e) {
		console.log(e)
		this.isScrolled = (e.scrollTop || 0) > 10
	},
	onShow() {
		...
		subordinateInformationApi(this.userInfo.userId).then((res => {
